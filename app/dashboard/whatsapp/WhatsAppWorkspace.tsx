...
type Data = { config: AgentConfig; threads: { thread: Thread; lead: Lead }[]; leads: Lead[]; queue: QueueItem[]; connection: { configured: boolean; connected: boolean; message: string; phone: string; qr: string }; calendar: boolean; lastTick: string | null; sentToday: number; workspaceId: string };
...
export default function WhatsAppWorkspace() {
  ...
  return <section className={styles.root}>
    ...
    {tab === "daily" && data && <div className={styles.daily}>
      <div className={styles.sectionHeader}><div><h2>30 passende Kontakte. Jeden Arbeitstag.</h2><p>Freigegebene Kontakte erhalten eine individuelle erste Nachricht. Antworten landen direkt in der Inbox.</p></div><span className={`${styles.badge} ${config.dailyOutreachEnabled && heartbeatLive ? styles.good : ""}`}>{config.dailyOutreachEnabled ? heartbeatLive ? "Tageslauf aktiv" : "Wartet auf Verbindung" : "Tageslauf pausiert"}</span></div>
      <div className={styles.dailyCards}><div className={styles.panel}><span>Heute versendet</span><strong className={styles.largeNumber}>{data.sentToday}<small> / {config.dailyOutreachLimit}</small></strong><progress value={data.sentToday} max={config.dailyOutreachLimit} /><p>{activeQueue} Kontakte warten auf den nächsten Versand.</p></div><div className={styles.panel}><h3>Versand steuern</h3><p>{config.outreachStartHour}:00–{config.outreachEndHour}:00 Uhr · {config.timezone}<br />Höchstens eine neue Nachricht alle zwei Minuten.</p><button className={config.dailyOutreachEnabled ? styles.secondary : styles.primary} disabled={Boolean(busy) || (!config.dailyOutreachEnabled && !data.connection.connected)} onClick={() => void saveConfig({ ...config, enabled: true, dailyOutreachEnabled: !config.dailyOutreachEnabled })}>{config.dailyOutreachEnabled ? "Tageslauf pausieren" : "Tageslauf aktivieren"}</button><button className={styles.textButton} onClick={() => setTab("knowledge")}>Limit & Regeln bearbeiten</button></div></div>
